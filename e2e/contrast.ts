 ...
 *    (`.bad`, `.good`, `.warn`), the `.mechanism` frame, every painted compare
 *    cell (`.mech-cell--match`, `--mismatch`, `--scanned`), the one-bits in the
 *    exponent row (`.mech-bit--1`), the data-table header row, `.links a` and
 *    the hero aside. Every `button` and `.panel` is a `linear-gradient`, and
 *    `html, body` is two `radial-gradient` washes over `var(--bg)`. axe resolves
 *    none of them and files them under `incomplete`, so a violations-only gate
 *    measured the contrast of almost nothing this lab paints.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This lab has four such
 *    rules and two of them are load-bearing teaching state:
 *    `.mech-cell--idle { opacity: .75 }` is every character not yet inspected,
 *    and `.mech-cell--skipped { opacity: .4 }` is the "never inspected" class
 *    that IS the leak the exhibit exists to show. The other two are
 *    `.cl-hero-sub { opacity: .85 }` and the `opacity: .7` on a busy/disabled
 *    run button.
 *
 ...
 * the walk as a pair rather than fading the foreground alone. `.mech-cell` sets
 * its own `background: var(--panel)`, so this is exactly the shape the pair
 * models: at `opacity: .4` the cell's fill fades too, and the ink lands on the
 * mechanism panel showing through it.
 *
 ...
 *    boxes are `.mech-grid` (`overflow-x: auto`, `min-width: 0`) and
 *    `.chart-data` (`overflow-x: auto`). The first is the character-by-character
 *    compare grid — 837px of `flex-wrap: nowrap` cells — and at 380px most of it
 *    is genuinely outside the frame; the second holds the measured-data tables.
 *    Content scrolled past either is not dimmed or partly drawn — it is absent,
 ...
 *    draws its charts into `<canvas>`, and both mechanism figures are CSS
 *    flex/grid rows of `<span>`s, so the sibling walk finds nothing to do. Its
 ...
 * not one — nor is it one to axe's `color-contrast` rule. This lab has three
 * such rules and every one of them draws the NON-COLOUR CUE the mechanism
 * exhibits exist for: `.mech-cell[data-mark]::after` paints the ✓ / ✗ / – / •
 * above each compare cell, and `.mech-bit[data-mark]::after` paints the ×
 * marking the extra multiply on a 1-bit. Strip them and the exhibits convey
 * their state by colour alone. `nontext.ts` measures generated content directly
 * and runs at every driven state, so these are gated rather than hand-measured.
 *
 * CANVAS PIXELS. All four charts are `<canvas>`, and nothing in either oracle
 * can read a pixel a 2D context painted. The charts are decorative duplicates of
 * data that is also rendered as a real `<table>` inside each panel's
 * `<details>` — the gate opens all four and measures those tables — but the
 * chart ink itself is outside both oracles and is stated here rather than left
 ...
 * only motion is the two mechanism steppers, which are `setTimeout` chains that
 * repaint `.mech-cell` / `.mech-bit` classes rather than CSS animations, and
 * under the reduced motion this gate asserts `animate.ts` skips the stepping
 * entirely and calls `finish()`, so every cell is already at its end class
 ...
 * sometimes-measured assertion is worse than none. The Replay buttons
 * deliberately bypass the reduced-motion check (an explicit press is a request
 * to SEE the animation), and that is the one place here where such a state
 * exists.
 */
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * too: `html, body` carry two `radial-gradient(circle at …)` washes — teal
     * at 15% 10% and rust at 90% 0% — over `var(--bg)`, and BOTH boxes declare
     * them, so the canvas and the body paint the same pair. Every `button` is a
     * `linear-gradient(160deg, …)` as well.
     */
     ...
     *
     * A driven pass here walks five panels at once, and the expensive part is
     * that `animate.ts` renders ONE `<span class="mech-cell">` PER CHARACTER of
     * both the secret and the guess, plus one `<span class="mech-bit">` per
     * exponent bit, and each measured-data `<table>` is one `<td>` per cell —
     * hundreds of sibling spans all re-walking the same ancestors up to
     ...
     * that is `.mech-grid` (`overflow-x: auto; min-width: 0`), which holds the
     * nowrap character cells of the compare walk, and `.chart-data`
     * (`overflow-x: auto`), which holds each panel's measured-data table.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `.chart-data` disclosures, one per measuring panel, all of which ship
      // shut and each of which holds that panel's measured-data table — the only
      // rendering of the numbers that is not a `<canvas>`. The gate opens each
     ...
     *
     *  - `.mech-bit`, each digit of the exponent-bit row in Panel 3
     *    (`animate.ts`'s `buildBitRow` sets `aria-hidden="true"` on every cell).
     *    It is hidden deliberately: the row's host carries
     *    `role="img"` with `aria-label="Exponent bits; ones trigger an extra
     *    multiply"`, so exposing forty individual "1"/"0" spans underneath it
     *    would read the label and then spell the bits one at a time. The tallies
     *    below it — "N squares · M multiplies" for both schedules — are NOT
     *    hidden and ARE measured, and they carry the same fact in words.
     *    Its ink is `--text` on `--panel` for a 0-bit and on
     *    `color-mix(in srgb, var(--accent-2) 24%, var(--panel))` for a 1-bit.
     *    The × mark above a 1-bit is a `::after`, which `nontext.ts` measures
     *    directly and reports with "host is aria-hidden" attached.
     *
     ...
     *
     * This lab has no SVG of its own — the charts are `<canvas>` and both
     * mechanism figures are flex rows of `<span>`s built in `animate.ts`. The
     ...
     * five-panel document many screens deep the root's own border box ends long
     * before the content does. Without the propagation rule below, every element
     * past the first screen would find no ancestor paint at all and fall through
     * to WHITE, which in the dark theme (`--bg: #1e232b`) reports near-white ink
