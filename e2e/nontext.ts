 ...
 *
 * BOTH halves are live in this repo, and the second one is the reason this file
 * matters more here than in most labs.
 *
 * The first half checks a claim the stylesheet makes about itself. `main.css`
 * defines `--border-strong` with a comment asserting specific ratios — "4.37:1
 * on --panel, 3.97:1 on --bg and 3.35:1 on --bg-soft" — and applies it to the
 * shared `button, input` rule. A comment is not evidence: it was written for the
 * flat `--panel` fill, while every `button` on this page actually paints a
 * `linear-gradient(160deg, color-mix(in srgb, var(--accent) 18%, var(--panel)),
 * var(--panel))` over a `.panel` that is itself a gradient, and the two
 * mechanism Replay buttons sit on a third surface again
 * (`color-mix(in srgb, var(--accent) 6%, var(--panel))`). This oracle measures
 * the composite each control is really drawn on, at every driven state,
 * including `:hover` — which persists after a click and is the state a reader
 * occupies the instant after pressing Run.
 *
 * The second half is the one this lab cannot do without. Three `::after` rules
 * paint the NON-COLOUR CUE that the mechanism exhibits exist to provide:
 * `.mech-cell[data-mark]::after` draws ✓ / ✗ / – / • above each character of the
 * compare walk, and `.mech-bit[data-mark]::after` draws the × that marks the
 * extra multiply on a 1-bit. `animate.ts` says in as many words that "every cell
 * is styled by icon/text AND colour (never colour alone)" — those marks ARE the
 * icon half of that claim, and neither axe nor the arithmetic walk can see a
 * single one of them.
 *
 ...
 *    That is exactly the shape `main.css` warns about for `input`: it is filled
 *    with `--panel`, the same colour as the card behind it, so its border is the
 *    ONLY thing that makes the control visible.
 *
 ...
 *    the wrong surface, and `escaped` is reported so triage can see it. Both
 *    mark rules here ARE `position: absolute` at `top: -0.85rem`, so every mark
 *    finding from this lab lands in that category and is hand-measured against
 *    what the mark actually overlaps — the `.mechanism` frame, not the cell.
 *
 ...
 * background beneath — because a control's fill here is a gradient over a
 * `color-mix()` over a panel that is itself a gradient, and reading the declared
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * running the full height of a section several screens tall, so a control
     * near the top of a panel sits on a different surface than one near the
     * bottom. So each gradient is *sampled* at the control's real location
     ...
     * (`url()`, `none`) is unmeasurable and paints nothing. The radial branch is
     * load-bearing here too: `html, body` carry two `radial-gradient(circle at
     * …)` washes — teal at 15% 10% and rust at 90% 0% — over `var(--bg)`. Every
     * `button` in this lab is itself a `linear-gradient(160deg, …)`, so the
     * linear branch runs on the fill of every control judged below.
     */
     ...
     * the generated-content half: it asks for TWO extra computed styles
     * (`::before` and `::after`) on every element on the page, and `animate.ts`
     * renders one `<span>` per character of the secret and the guess plus one
     * per exponent bit. Without the caches the pass re-reads the same computed
     ...
     * what a page paints. This lab is a direct instance of the failure: `html,
     * body` declare their surface as a three-layer `background` shorthand — two
     * radial washes over `var(--bg)` — so `backgroundColor` reads
     * `rgba(0, 0, 0, 0)` on BOTH boxes. A `background-color`-only walk therefore
     * finds nothing opaque anywhere, falls through to WHITE, and judges every
     * control on the page against a page that does not exist. Elsewhere in this
     * fleet that exact bug reported a bright disclosure triangle at 3.95:1 over
     * a near-black canvas, and the tell was that the number came out IDENTICAL
     * in both themes — a ratio that cannot depend on the theme is not measuring
     * the theme. It borrows `contrast.ts`'s gradient-aware sampler wholesale
    ...
    const CONTROL = [
      ...
      // control?" test below. This lab's Panel 5 `.links a` chips are pill-shaped
      // and filled, but they carry neither `btn` nor `button` in their class, so
      // they are judged as prose links; their fill is measured all the same by
      // the 1.4.3 walk in `contrast.ts`, which reads the ink sitting on it.
      'a[role=button]',
      'a[class*=btn]',
      'a[class*=button]',
    ].join(',');
